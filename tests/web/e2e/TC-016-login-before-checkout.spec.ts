...
test.describe('TC-016: Place Order: Login Before Checkout', () => {
    ...
    test.beforeEach(async ({ page, homePage, signUpLogInPage, signupPage, accountCreatedPage }) => {
        ...
        await test.step('Verify account created and clear the session to log out', async () => {
            await expect(
    ...
    test('should allow a logged-in user to complete an order', async ({
        page,
        homePage,
        cartPage,
        productsPage,
        ...
        await test.step('Open the login page and log in', async () => {
            await homePage.navigationMenu.clickSignupLogin();
            await expect(
                signUpLogInPage.newUserHeader,
